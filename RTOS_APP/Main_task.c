...
void Main_init(void)
{
	// SoftSPI_OLED_Init();           // 已移除，无SoftSPI OLED
	// SoftSPI_OLED_Clear();          // 已移除
	// SoftSPI_OLED_DisplayTurn(0);   // 已移除
	visual_idle();
	pos_reset();
}
...
void Show_Task_Code(void)
{
	...
	// {
	//     SoftSPI_OLED_ShowBN(8+i*16,16, color_task[i], 1);  // 已移除
	// }
	// SoftSPI_OLED_ShowBN(56,16, 0, 1);               // 已移除
	// for(uint8_t i=3;i<6;i++)
	// {
	//     SoftSPI_OLED_ShowBN(24+i*16,16, color_task[i], 1); // 已移除
	// }
	// SoftSPI_OLED_Refresh();                          // 已移除
}
...
//	while(1){osDelay(50);};
	...
	// SoftSPI_OLED_ShowString(16,0,(uint8_t *)"SW Ver 2.7.4",16,1);   // 已移除
	// SoftSPI_OLED_ShowString(8,16,(uint8_t *)"OLED INTI PASS",16,1); // 已移除
	// SoftSPI_OLED_Refresh();                                          // 已移除

	while(!ready) { osDelay(10);}
	osDelay(100);
	// buzzer_rings(100, 0, 1);  // 已移除，无蜂鸣器
...
//		osDelay(500);
//
//		CRing_Dual_Calibration();  //色环双环校准

//			grab_materials_car(1, 0);     //底盘抓取物料  （第一个为序号）
//			put_materials_ground(color_task[0],0);
//			grab_materials_car(2, 0);     //底盘抓取物料
//			put_materials_ground(color_task[1],0);
//			grab_materials_car(3, 0);     //底盘抓取物料
...
//		while(1){osDelay(10);};
		...
		Chassis_Go_Pos(1050, 165, 0, 1, 300);  //T1路线

		Chassis_Go_Pos(1050, 1900, 0, 1, 300);  //粗加工区1

		Chassis_Go_Pos(1050, 1900, 90, 1, 50);  //粗加工区2
		...
		CRing_Dual_Calibration();  //色环双环校准
			...
			grab_materials_ground(color_task[0],1);     //底盘抓取物料
			put_materials_car(1);
			grab_materials_ground(color_task[1],0);     //底盘抓取物料
			put_materials_car(2);
			grab_materials_ground(color_task[2],0);     //底盘抓取物料
			put_materials_car(3);
		...
		Chassis_Go_Pos(1050, 1900, 90, 1, 200);  //粗加工区2
		...
		CRing_Dual_Calibration();  //色环双环校准
			...
			grab_materials_ground(color_task[3],1);     //底盘抓取物料
			put_materials_car(1);
			grab_materials_ground(color_task[4],0);     //底盘抓取物料
			put_materials_car(2);
			grab_materials_ground(color_task[5],0);     //底盘抓取物料
			put_materials_car(3);
	...
	while(1)
	{
	  uint8_t block_pos[3] = {0};
		...
		switch(VIS_RX.block_order)  //颜色对应色块
		{
